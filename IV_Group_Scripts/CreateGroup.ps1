...
# Check si le groupe existe deja
try {
...
try {
    $OU = [Microsoft.VisualBasic.Interaction]::InputBox("Entrez le nom de l'unité d'organisation (OU) où le groupe sera créé (par exemple, 'Domain Controllers').", "Create Group", "Domain Controllers")
...
# Portée du groupe (Group Scope)
try {
    $GroupScope = [Microsoft.VisualBasic.Interaction]::InputBox("Entrez la portée du groupe (par exemple, 'Global', 'DomainLocal' ou 'Universal').", "Create Groupe", "Global")
...
# Description du futur groupe
try {
    $DescriptionGroup = [Microsoft.VisualBasic.Interaction]::InputBox("Entrez une description au groupe '$GroupName' que vous voulez creer (laissez vide si vous ne voulez pas de description).", "Create Group", "En gros ici on fait de l'informatique je crois.")
...
try {
    ...
    Write-Host -ForegroundColor Green -BackgroundColor DarkGreen "Le groupe '$GroupName' a été créé avec succès dans l'unité d'organisation '$OU'."
} catch {
